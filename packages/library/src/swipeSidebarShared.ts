...
export const applyOpenPaneStyles = ({
  ...
  requestAnimationFrame(() => {
    ...
    const widthTransition = options.isAbsolute
      ? ""
      : `, width ${options.transitionMs}ms ${TRANSFORM_EASING}`;
    ref.current.style.transition = `${transformTransition}${widthTransition}`;
    if (child && options.fadeContent) {
    ...
    requestAnimationFrame(() => {
      ...
      } else if (ref.current.style.width !== `${options.sidebarWidthPx}px`) {
        // Ensure width is set once for absolute mode without animating it
        const prevTransition = ref.current.style.transition;
        ref.current.style.transition = transformTransition; // exclude width
        ref.current.style.width = `${options.sidebarWidthPx}px`;
